...
# from filter_depthmap_dynamic_objects import depthmap_mobile_objects_filter
        ...
        for cam in range(*cam_range):
            ...
            if generate_aggregated:
                lidar_agg_culling_path = BASE_mod + date + '/' + format(seq, '04') + \
                                         "/labeled/lidar_points/lidar_agg_culling/" + \
                                         format(frame, '010') + ".npy"
                lidar_agg_culling = np.load(lidar_agg_culling_path).reshape((-1, 3))
                # Project points onto image
                lidar_aggregate_projected = Lidar2Cam(lidar_agg_culling, T_IMG_CAM[cam], T_CAM_LIDAR[cam])
                # Crop points to image view field
                lidar_aggregate_cropped = CropPoints(lidar_aggregate_projected)
                # Generate depth map (H, W) from points (N, 3)
                depth_aggregate = GenerateDepth(lidar_aggregate_cropped)
                # depth_mobile_objects_mask = depthmap_mobile_objects_filter(annotation[frame], T_IMG_CAM[cam], T_CAM_LIDAR[cam])
                # depth_smart_aggregate = np.where(depth_mobile_objects_mask, depth_dror, depth_aggregate)
                # Save depth map to PIL
