...
class Kittiloader(object):
    ...
    """
    def __init__(self, kittiDir, mode, interp_method='nop', cam=2):
        self.mode = mode
    ...
    def _read_data(self, item_files, interp_method):
        ...
        if interp_method in ['nop', 'linear', 'nyu']:
            if interp_method == 'linear':
                data['depth_interp'] = interpdepth(depth).astype(np.float32)
            elif interp_method == 'nyu':
                image_data = rgb.convert('L')
                image_gray_arr = np.array(image_data)
                data['depth_interp'] = fill_depth_colorization(image_gray_arr, depth).astype(np.float32)
            else:
                pass
        else:
