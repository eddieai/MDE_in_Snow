...
if __name__ == '__main__':
    param = {
        ...
        'batch_size': 4,   # 3 if rescaled is False, 12 if True
        'eval_n_crop': 4,
        'learning_rate': 1e-3,
        'momentum': 0.9,
        'weight_decay': 0,
        'epochs': 30,
        'mode': 'sord_logphi'   # sord, sord_logphi, sord_ent_weighted, sord_min_local_ent, sord_weighted_minent, sord_align_grad, classification, regression, reg_of_cls
    }
    # train_type = 'refine'  # refine or continue
    model_dir = 'train_lr_1e-03_momentum_0.9_wd_0_epoch_30_mode_sord_logphi_pretrained_DeepLabV3+_PascalVOC_crop_375*513'
    ...
    model = net.get_model(param['mode'], pretrained=True)
    model.cuda()
